...
def parser_loader():
    ...
    parser.add_argument('--seed', default=3047,type=int,help="Please give a value for seed")
    parser.add_argument('--config',default='' ,help="Please give a config.json file with training/model/data/param details")
...
def print_pruning_percent(dataset_ori, dataset_pru, str1):
    # Substitute dgl with torch_geometric. 
    ori_all, pru_all = 0.0, 0.0
    for data_ori, data_pru in zip(dataset_ori, dataset_pru):
        ori = len(data_ori.edge_index[0])
        pru = len(data_pru.edge_index[0])
        ori_all += ori
        pru_all += pru
